...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await db.collection('jbi_gallery_categories').updateOne(buildIdFilter(id), { $set: updateData });
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await db.collection('jbi_gallery_categories').deleteOne(buildIdFilter(id));
    if (result.deletedCount === 0) return NextResponse.json({ success: false, error: 'Category not found' }, { status: 404 });

    await db.collection('jbi_gallery_items').deleteMany({ categoryId: id });
