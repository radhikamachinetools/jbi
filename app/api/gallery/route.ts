...
export async function GET() {
  try {
    ...
    const [categories, items] = await Promise.all([
      db.collection('jbi_gallery_categories').find({}).sort({ displayOrder: 1 }).toArray(),
      db.collection('jbi_gallery_items').find({}).sort({ displayOrder: 1 }).toArray(),
    ]);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === 'category') {
      ...
      const result = await db.collection('jbi_gallery_categories').insertOne(newCategory);
    ...
    if (type === 'item') {
      ...
      const result = await db.collection('jbi_gallery_items').insertOne(newItem);
