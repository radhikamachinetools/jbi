...
export async function GET() {
  try {
    ...
    const items = await db.collection('jbi_infrastructure').find({}).sort({ order: 1 }).toArray();
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { db } = await connectToDatabase();
    const count = await db.collection('jbi_infrastructure').countDocuments();
    const newItem = { ...body, order: count, createdAt: new Date() };
    const result = await db.collection('jbi_infrastructure').insertOne(newItem);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { db } = await connectToDatabase();
    await db.collection('jbi_infrastructure').updateOne(buildIdFilter(id), { $set: { ...body, updatedAt: new Date() } });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!id) return NextResponse.json({ success: false, error: 'ID required' }, { status: 400 });
    const { db } = await connectToDatabase();
    await db.collection('jbi_infrastructure').deleteOne(buildIdFilter(id));
    return NextResponse.json({ success: true });
  } catch (error) {
