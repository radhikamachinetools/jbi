...
async function getAllProducts(): Promise<Product[]> {
  try {
    ...
    const products = await db.collection('jbi_products').find({ status: { $ne: 'inactive' } }).sort({ order: 1 }).toArray();
...
async function getCertificates(): Promise<Certificate[]> {
  try {
    const { db } = await connectToDatabase();
    const certificates = await db.collection('jbi_certificates').find({ status: 'active' }).sort({ displayOrder: 1 }).toArray();
        ...
        <div className="container mx-auto px-3 sm:px-6 lg:px-8">
          ...
          <div className="relative h-64 lg:h-96 bg-gray-900 overflow-hidden">
            <Image
              ...
              alt="Jai Balaji Industries Factory"
              fill
